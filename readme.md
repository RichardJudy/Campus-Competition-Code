...
校赛用的装甲板检测系统，基于 USB 相机和传统视觉算法，能够实时检测装甲板并输出云台坐标、距离和 yaw/pitch 角度。
...
请参考 [相机标定指南](CALIBRATION_GUIDE.md) 进行相机内参标定。
...
```bash
./build/usbcamera_detect_test configs/demo.yaml -n video0 -d
```

#### 在 build 目录运行

```bash
./usbcamera_detect_test ../configs/demo.yaml -n video0 -d
```
...
```
装甲板 #123: 云台坐标(1.234, 0.567, 2.890), 距离=3.456m, yaw=5.23deg, pitch=-2.15deg
```
其中：
- `#123`: 帧序号
- `云台坐标(1.234, 0.567, 2.890)`: 目标在云台坐标系中的坐标 (x, y, z)
